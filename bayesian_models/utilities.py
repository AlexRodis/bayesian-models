...
#     '''
#         Calculate the Dirichlet distributions' critical moments. A dirichlet 
#         is fully specified by it's first two moments, i.e. mean and variance
...
#     import numpy as np
#     INPUT_TYPE=str(type(a)).split("'")[1]
#     independant_dim=a.dims[0] if INPUT_TYPE ==\
#         'xarray.core.dataarray.DataArray' else 'dim_0'
#     Z = 3 if standardize else 2
#     local_attrs=dict(created_on=datetime.datetime.now(),
#                                 memory_size="{size:.2f} GiB's".format(
#                                     size=np.prod(a.shape[1:])*Z*64*1e-9))
#     if INPUT_TYPE!='xarray.core.dataarray.DataArray':
#         try:
#             dat=a if INPUT_TYPE == "numpy.ndarray" else a.values
#             dims=a.shape
#         except AttributeError:
#             raise ValueError((f"Unable to coerce input {type(a)} into "
#             "an array-like"))
#         α=xr.DataArray(dat,dims={f'dim_{i}':range(dim+1) for i,dim in \
#             enumerate(dims)}, attrs=local_attrs)
#     else:
#         α=a
#         α.update_attrs(dict(α.attrs)|local_attrs)
#     α0=α.sum(dim=independant_dim,keep_attrs=True)
#     μ=(α/α0)
#     var=α*(α0-α)/(α0**2*(α0+1))
#     dir_moments=xr.Dataset(data_vars=dict(
#         mean=μ,
#         variance=var,
#         ),attrs=α.attrs)
#     if standardize:
